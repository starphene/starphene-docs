Project Town Game

Plans:

Time machine
Add time machine which allows fast forwarding time right from the get go. Each ad gives access to it for 60 seconds, plus you get 60s freebie. Time outside the game gives you a small amount of access to it with logarithmically more gains from more time, but if the game detects you’re cheating (the clock time remains the same but the day changes, long term delays don’t differ enough, too much time is skipped) it silently and gently reduces timescumming effectiveness (to ~50% after 10 times) until a cap. You get infinite access to it with premium. Non-premium decelerates pace faster and silently

The disease system 
Would be communicated clearly. The herbs will take disproportionately much resources to gather but diseases will be rare and the storage limits would be generous, requiring you to produce and stockpile herbs to avoid consequences. There would be random events which would force the player into choices into wasting large chunks of their supply to get sweet bonuses or to get nothing. Failure to address the issue will either amount to severely capped population loss (33% maybe) and/or make recovery fast and artificially supported, plus there would generally be productivity efficiency decrease while population is recovering. There would be a pretty wide variety of diseases with some having interesting effects like forcefully aging or deaging the population. As the game progresses, so would diseases, but required older medicine won’t be as effective on them anymore.

Random events 
Similar model to after inc, nothing to explain

Food
Starts as a basic resource before branching out into different types of nutrients, incentivizing varied production. Feeding the population all available types of food would give productivity buffs. Starvation cascades will be made way milder by an ability to turn on rationing mode and prioritize abandoning food production last in case of depopulation (it’d also be a whole system configurable by the player). Likewise, there would be an option to increase consumption to grow population faster unlockable with the tech tree. Industrial age would introduce happiness loss from unemployment.

Happiness
High value boosts birth rates and productivity, low value plunges productivity and explorer/warrior efficiency, negative value activates discord mode and causes people to leave. Maintained by taking care of the population. Eventually buildings are added to help it. Progression forces player to do more to maintain it. Would be split into a volatile value directly influenced by bonuses/demerits, the actual value which would approach the volatile one, and the approach speed (delta) dictated by discord/fatigue values and possibly a few external buffs.

Discord mode and crisis fatigue (bar)
Activates when happiness or city security plummets too far into the negatives, and is stopped when volatile happiness/security is restored to 60%, with lower cancelation thresholds the longer discord mode lasts. Aimed at preventing crises from being too punishing. Has an orange mode (discord mode) and a red mode (chaos mode? Super discord mode?), both of which add color specific elements to UI to show tension without GTA online loss streak bullshit. Orange mode can turn into red, but not the other way around. Orange freezes happiness delta by 50% and red freezes it completely, both give bonuses to productivity and reduce security resource cost (and even more so to combat efficiency) but red gives more. Crisis fatigue is a bar which drains during discord, with red discord draining it 2.5x faster (entering both orange and red discord gives a deduction from crisis fatigue every time it's triggered). If the bar runs out, it can go into negatives up to a certain point (-25%), and that would cause the compensatory buffs to gradually recede over time until they hit zero, but that would be reasonably rare and mostly aimed at nudging the player against abusing combat benefits. Crisis fatigue has a large pool and is recovered faster the better is happiness, but recovers very very slowly as a baseline.

Population and housing
Bottlenecks how much production can be active in production and how much rare public servant/fighter candidates are there. Housing increases max population. If max pop < current pop, people slowly leave. This is modified with latter research, allowing population growth above max, but with worsening happiness debuffs. Early-mid game will introduce different types of housing with different advantages for the player to pick.
Stored in the memory as kids/working age/starting to age/too old to work.

Resources
Wood (buildings)
Stone (buildings, other resources)
Fur (building material, used for armor, later tech tree upgrade to remove it from the game)
Energy (can be allocated to empower production of certain buildings multiplicatively)
Food (needed to sustain population growth and happiness, can be rationed in times of crisis or allowed overconsumption of to boost birth rates)
Elite food (can be eaten to restore energy, also used by combatants and public servants to grow their happiness)
Energy (used to boost certain buildings' productivity)
Sugars (increase population loyalty)
Security (small volume, restores quickly, used to prevent minor incidental resource/people loss and is consumed during some random events. Restores progressively slower when in the negatives and triggers discord mode when too far in the negatives)
Copper (used for weapons, armor and jewelry, and later electronics, can later be transmuted from stone)
Gold (used as a currency, for jewelry and to enhance weapons and armor)
Gems (for jewelry and to enhance weapons and armor)
Iron (used for weapons and armor)
Silver (used for weapons, armor, jewelry, and yes it’s above gold somehow)
Platinum (ultra currency for special deals and panic buttons)
Gunpowder (powers weapons and some buildings)
Oil (medicine, energy, happiness buildings, later weapons, armor and space travel)
Aliminium (used for weapons, armor)
Isotopes (energy, space travel and later mechanoids)
Magnetogens (combines rare earth magnetogens, powers weapons, armor and mechanoids)
Plasteel (powers ai age tier buildings and mechanoiods)
Efferprism (powers supreme mechanoids and space travel)

Tech tree
It will be huge and awesome. Will include:
New buildings
New manual gathering actions
Passive bonuses 
New mechanics
Allowing pop growth past max, adding homelessness
Allowing rationing to survive crisees
Allowing overeating to accelerate birth rates (two tiers)
Lowers the minimum working age but decreases happiness
Lowers the maximum working age but decreases happiness
Would have an option to branch into ruling with an iron fist to minimize worrying about happiness through maintaining discord mode, ruling with consideration of the people’s needs and maximizing happiness, and ruling neglectfully to maximize profits through ignoring happiness
Will be accessible through pulling up the swipe left menu. The background would be an image of a starry sky, and on it would be a long chain of nodes symmetrical around the y axis going from bottom upwards. The node manager would generate each node according to my incoming info about each node, and each connected node would have an white or green arrow connecting them in case of requirements (the color depends on if the requirement is met). The nodes would be pretty minimalistic in design with a type of gain from the node on the icon, and the border indicating whenever it's complete or not. Tapping on any node would pull up a popup with its description with all its relevant information, and have a button which unlocks the node's effects when held up for 1.5 seconds. 

Actual tech tree:
Stone age: 
Unlock building: living hut (base pop building), also gives a popup to explain the population/housing mechanic
Unlock building: forager's hut (auto food gathering)
Unlock building: logger's hut (auto wood gathering)
Unlock building: stonepicker's hut (auto stone gathering)
Buff: manually picking wood produces 3x as much of it
Buff: manually picking food produces 3x as much of it
Buff: manually picking stone produces 3x as much of it
Unlock action: Gather Herbs (tier 1 medicine), also gives a popup to explain the disease mechanic
Unlock building: Healer's hut (picks herbs OR heals population)
Unlock building: hunter's hut (auto food and more of it, auto furs)
Unlock building: stockpile hut (increase storage size for most resources)
Unlock building: researcher's hut (reduces research resource cost)
Buff: foragers' huts produce 15% more food for 15% less work (all multiplicatively)
Buff: stonepicker's huts take 20% more work produce stone 40% faster (all multiplicatively)
Buff: logger's huts produce wood 20% faster (multiplicatively) and increase max wood storage by (flat X) each.
Buff: stockpile huts are now 60% cheaper to build (multiplicatively)
Buff: bonuses from researcher's huts are now 30% higher, but they now require 35% more work (all multiplicatively)
Buff: hunter's huts now produce 2x as much fur and are now 30% cheaper to build (multiplicatively)
Unlock action: Rationing mode (halves food consumption when turned on and reduces birthrates multiplicatively by 3. However, activating it outside of a crisis will trigger one, making the population pretty unhappy), also gives a popup to explain discord mode
Buff: The villagers have started to recover from their rocky start of our rule. Crisis fatigue recovers 50% faster multiplicatively.
Unlock building: Chieftain's cabin (player's home, unlocks energy, allowing you to allocate it to empower certain buildings' production)
Buff: handpicking any resource adds 1 different resource to the stockpiles, sometimes including furs and herbs
Unlock building: Villager's cabin (way more efficient housing)
Unlock building: Hunter's cabin (auto food, auto furs, way faster and more efficient than huts)
Unlock building: Farm (auto food in enormous amounts, possibly 4x forager hut's output)
Unlock building: Logger's cabin (auto wood, way more efficient production)
Unlock building: Lecture cabin (way better research resource cost reduction, each cabin also empowers all production speed by 5% additively)
Unlock building: Mine (way faster stone production)
Unlock action: Dungeon crawling, diablo style itemization for candidates for exploration and their equipment, also unlocks dungeon Slimy mines (from enough stuff produced from the mines, also meeting the milestone requirement for this tanks output of mines by 30% until the dungeon is cleared;)
(initially dungeon crawling only allows 2 party members and only a few classes but the mechanical foundation is already being set here. The dungeon is a literal straight line until halfway through it splits into two directions. Each direction leads to a miniboss, and each miniboss cell would have a one way path back to the crossroad. Killing any of the minibosses turns the other one into the dungeon boss)
Unlock action: Crafting (weapons and armor, they're a lot tamer and harder to hit an item with rarities above normal but can be good to make up for underleveled gear)
Unlock building: Altar (buffs explorers and increases happiness)
Unlock action: when allowing explorers to fully self-manage you get options to make them prioritize loot, progress or map clarity
Unlock action: explorers note down the cells they visited before, noting them on a map accessibleto the player
Buff: explorers have additively more defense, getting full healed increases their max hp by 10% for the duration of the run and now have this increase incremented by other buffs
Buff: explorers have a base 10% chance to crit and now have this increase incremented by other buffs

Copper age:
Unlock action: mirrored infinity (defeating the dungeon's boss punches a new path in a random wall which allows entrance to the mirrored infinity. it allows the explorers to explore an infinite mirror of the greater dungeon which increases in difficulty linearly and in rewards in square as the party progresses further)
Buff: Mirrored infinity loot scales in cube instead of square
Buff: 15% chance to not consume any medicine when the party is defeated, and this increase is incremented by other buffs
Unlock building: Smelting Pit (consumes wood and copper ore to smelt copper bars)

this dungeon in particular is shaped like a wheel with the entrance being on the side, it would require killing 2 slime minibosses to breach the middle and unlock
Unlock building: fisherman's house (auto food and occasional treasure)

Steel age:

Magic age:

Silver age:

Industrial age:

Information age:

AI/singularity age:

Progression timeline:
Game start (~-6000BC) Energy, wood, stone, tech tree, time machine, population and housing, food, fur, tier 1 diseases and meds, stockpile hut, discord mode/crisis fatigue (when player naturally reaches that point) 
Copper age (-2500BC) Dungeon crawling, copper, gold, relics (rare items which appreciate in value indefinitely over centuries), guards system 
Steel age (-400BC) Iron, townsmen style trading, public servants system 
Magic age (400) Tier 2 diseases and meds, elite food, guest public servants, gemstones Silver age (late medieval, early colonial, 1200) 
Silver age, split of food into the protein/carb/fat/fiber (possibly with only a skill tree upgrade since that might overburden the player), platinum 
Industrial age (1700) Gunpowder, guns, Electricity, lithium, Tier 3 diseases and meds, unemployment debuffs, drugs, oil 
Information age (1950) 3 (4?) new skill trees which give bonuses through high happiness, bonuses through maximizing profits and neglecting happiness while keeping it above zero, and bonuses through staying in crisis mode perpetually, aluminium, limited space travel mostly for show 
AI/singularity age (2020) Plasteel, Tier 4 diseases and meds, ability to make robots to replace people, asteroid mining, atomic fabrication, cybernetical augmentations


Electricity
Produced by power plants, powers pretty much every building starting from industrial era

Energy, manual work and rallying workers
The player has a pool of energy which restores quickly but has a small max amount, allowing to assist in production. At some point this gets replaced with the player being given options to automatically enchant certain buildings’ production after the town gets too big. You can spec into maximinning or minmaxing the bonus from this effect.

Story: The player is an immortal vampire. They have an enigmatic, obedient councilor who basically does everything for them. 

Dungeon crawling
Similar model to magic research, has resources which are originally exclusively obtained from there but later gain a way to produce them with a building. Dungeons also often pose an active threat and occasionally generate negative random events before they’re cleared out and cease posing a threat. There are several possible classes with specific synergies between one another. Each class has separate skills, strengths, weaknesses, and equipment needed. I’ll try to make manual combat interesting through some fun, redoable puzzle the player would have to solve. Equipment has deep itemization and is gathered through finding it or crafting it.
The dungeon crawling system which would be designed around a procedually generated grid of cells, with latter dungeons having more complex shapes and mechanics. Each cell would give the player a fight, a shop using a dungeon-only currency, a full heal, a loot chest and possibly a few other possible effects. After clearing out a cell the player be forced into a choice where to go after most of the battles, and the game will automatically map what the player has explored. The entire sent out party dying will send all collected items and keep all the progress on changing or exploring cells but will deduct a maintenance cost from the resources before the party is allowed to continue. Each dungeon has a specific way to reach the boss, and killing the boss would give the player an additional extra reward and possibly unlock the next dungeon in the list. The combat itself would probably be entirely strategy and equipment based with little player input on the fight outcome after that. There would also be an option to delegate all the decision making to the party which would explore the dungeon autonomously based on the player-set priorities of progress or loot. 
Cell types:
Enemy (gray) 
Shop (blue) 
Full heal (green)
Loot (yellow, turns into enemy cell after you take the loot) 
Gray with red outline (switch required to unlock boss guarded by an enemy) 
Red (boss) 
Purple (door to mirroring infinity, a place where the party can explore an infinite procedually generated subdungeon for loot, though the amount of loot and shop cells is considerably smaller there, and there's a reasonably rare chance to find a ) 




Public servants (have rarity tiers, give different bonuses depending on the exact type. Have their own happiness which tanks their bonuses which is affected by the salary and access to premium commodities. However, they get used to having these mood boosts which moves the bar for them upwards (but the distance between standards and actual satisfaction requirement still slowly increases so the player doesn't feel as cheated by upgrading their accommodations), and if the level of accomodation drops below their bar the productivity tanks and they need 3 times as long to lower the bar than they need to rise it. Might also make the growth pattern toward target bar distance hyperbolic (or x^-0.5) so waiting always brings benefits) Dungeon crawling (need employees and gear for them, used to clear out threats to the city when they appear and regular dungeons which can be explored for resources, loot, etc etc. Employees, their gear, and enemies have difficulty tiers. Might make a cool minigame for manual combat) Artists (convert specific groups of resources into art pieces which can be sold or treasured depending on the quality) Trade routes (ability to make planned exchanges of overproduced shit for underproduced shit, but being unable to make a preordained purchase damages trust and temporarily reduces the amount of shit you can order in advance. Population + average happiness affects) Rarity (every resident has a rarity category in each of the possible skillsets which goes from gray (weak) - white (common) - green (uncommon) - teal (lucky) - blue (rare) - yellow (extraordinary ) - orange (unique) - red (legendary) - purple (mythical) - rainbow (supreme). All residents have a random age where their skills plummet due to old age and another one where they die, with yhe player being able to upgrade life expectancy. As population grows, access to higher rarities becomes more available, but occasionally it's possible to get a rarity 2 tiers above normal. Residents of tiers way below progression norm are not displayed to save performance. The rng allocation is specifically built so a lot of amazing residents have several aptitudes where they're amazing to force the player into difficult choices. It's possible to buy high-tier residents if you get lucky and if you give them enough of an entry bonus, but they're also way more sensitive to bad treatment and can even leave. Gear also has the same rarities) Tech tree (requires specific buildings and workers for it and handles lots of unlocks, is gated by progress in other areas)



Stages of sentient life congregations:

Village - County - Region - Country - Hegemon  - Planet - System - Star Cluster (20-2000 systems) - Subsector (3-300 clusters or 2k-100k systems) - Sector (5-800 subsectors or 100k-50m systems)

